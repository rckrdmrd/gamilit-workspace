...
export const MatchingCard: React.FC<{ card: CardType; isSelected: boolean; onClick: () => void }> = ({ card, isSelected, onClick }) => (
  <motion.div
    ...
    className={cn('p-6 rounded-lg text-center font-semibold cursor-pointer min-h-[120px] flex items-center justify-center',
      card.isMatched ? 'bg-detective-success/10 text-detective-success opacity-50' : isSelected ? 'bg-blue-500 text-white ring-4 ring-blue-300' : card.type === 'question' ? 'bg-orange-100 text-orange-800 hover:bg-orange-200' : 'bg-purple-100 text-purple-800 hover:bg-purple-200'
    )}
  >
    {card.content}
  </motion.div>
