...
export const VerdaderoFalsoExercise: React.FC<VerdaderoFalsoExerciseProps> = ({
  ...
  const { submit, isSubmitting, recordHintUsed } = useExerciseSubmission(exercise.id, {
    onSuccess: (result) => {
  ...
  const handleCheck = () => {
    ...
    const answers = statements.reduce(
      (acc, stmt) => ({
        ...acc,
      ...
      <DetectiveCard variant="default" padding="md" className="mb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <ProgressTracker currentStep={answeredCount} totalSteps={statements.length} />
          </div>
          <div className="flex items-center gap-3">
            <HintSystem
              hints={exercise.hints.map((h) => h.text)}
              onHintUsed={(idx) => {
                const hint = exercise.hints[idx];
                if (hint) handleUseHint(hint);
              }}
            />
            <DetectiveButton variant="blue" onClick={handleReset} disabled={answeredCount === 0}>
              Reiniciar
            </DetectiveButton>
            <DetectiveButton
              variant="gold"
        ...
        <AnimatePresence>
          {statements.map((statement, index) => {
            ...
            return (
              ...
              >
                <DetectiveCard
                  variant={showResult ? (result.isCorrect ? 'success' : 'danger') : 'default'}
                  padding="md"
                    ...
                    {showResult && result.explanation && (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <p className="mb-2 text-detective-text">{result.explanation}</p>
                            <p className="text-sm text-detective-text-secondary">
                              Respuesta correcta:{' '}
                              <strong>{result.correctAnswer ? 'Verdadero' : 'Falso'}</strong>
                            </p>
                          </div>
                        </div>
                      </motion.div>
