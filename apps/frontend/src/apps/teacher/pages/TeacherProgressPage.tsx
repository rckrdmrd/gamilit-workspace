...
type ProgressTab = 'progress' | 'engagement';

const PROGRESS_TABS: TabDefinition<ProgressTab>[] = [
...
/**
 * TeacherProgressPage - Pagina de seguimiento de progreso academico
 *
 * Proporciona una vista integral del progreso de los estudiantes con:
 * - Selector de clase para filtrar datos
 * - Dashboard de progreso con graficos y metricas
...
export default function TeacherProgressPage() {
  ...
  useEffect(() => {
    ...
    if (classroomIdFromUrl) {
      ...
      if (exists) {
        ...
        toast('La clase solicitada no existe o no tienes acceso. Mostrando todas las clases.', { icon: '\u26A0\uFE0F' });
      }
      return;
    }
  ...
  const exportToCSV = async () => {
    if (selectedClassroomId === 'all') {
      toast('Por favor selecciona una clase especifica para exportar', { icon: '\u26A0\uFE0F' });
      return;
    }
    ...
    try {
      ...
      if (report.status === 'completed' && report.file_url) {
        ...
        toast('El reporte esta siendo generado. Por favor intenta nuevamente en unos momentos.', { icon: '\u2139\uFE0F' });
      }
    } catch (err) {
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="relative">
                ...
                {showClassroomDropdown && (
                  <div className="bg-detective-card border-detective-border absolute left-0 right-0 top-full z-50 mt-2 max-h-80 overflow-y-auto rounded-lg border-2 shadow-detective-lg" role="listbox" aria-label="Clases disponibles">
                    {/* All Classes Option */}
                    <button
                      onClick={() => {
                        setSelectedClassroomId('all');
                      ...
                      className={`w-full px-4 py-3 text-left transition-colors hover:bg-detective-bg-secondary ${
                        selectedClassroomId === 'all'
                          ? 'bg-detective-orange/10 font-semibold text-detective-orange'
                          : 'text-detective-text'
                      }`}
                    >
                      <div>
          ...
          <>
            {selectedClassroomId !== 'all' ? (
              ...
              <DetectiveCard hoverable={false}>
                <div className="py-16 text-center">
                  <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-detective-orange/10">
                    <BarChart3 className="h-8 w-8 text-detective-orange" />
                  </div>
            ...
            <DetectiveCard hoverable={false}>
              <div className="flex items-start gap-4">
                <div className="bg-detective-accent/10 flex-shrink-0 rounded-lg p-3">
        ...
        {!loading && !error && activeTab === 'engagement' && (
          <div className="space-y-6">
            ...
            <DetectiveCard hoverable={false}>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <DetectiveButton variant="secondary" onClick={refreshAnalytics} aria-label="Actualizar metricas de engagement">
                    <RefreshCw className="h-4 w-4" />
                  </DetectiveButton>
                </div>
              </div>
            ...
            {engagementData && selectedClassroomId !== 'all' && !analyticsLoading && (
              <div role="region" aria-label="Metricas de engagement">
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {(engagementData?.comparison_previous_period?.dau_change ?? 0) >= 0 ? '+' : ''}
                          {safeFormatNumber(engagementData?.comparison_previous_period?.dau_change, 1, '%', '0.0%')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {(engagementData?.comparison_previous_period?.wau_change ?? 0) >= 0 ? '+' : ''}
                          {safeFormatNumber(engagementData?.comparison_previous_period?.wau_change, 1, '%', '0.0%')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {(engagementData?.comparison_previous_period?.engagement_change ?? 0) >= 0 ? '+' : ''}
                          {safeFormatNumber(engagementData?.comparison_previous_period?.engagement_change, 1, '%', '0.0%')}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {engagementData?.feature_usage && engagementData.feature_usage.length > 0 && (
                  <DetectiveCard hoverable={false}>
                    ...
                    <DataTable<{ feature_name: string; usage_count: number; unique_users?: number }>
                      data={engagementData.feature_usage.filter(
                        (f) =>
                          f &&
