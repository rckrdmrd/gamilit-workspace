...
/**
 * useTeacherPageSetup - Centralizes teacher page boilerplate
 *
 * Encapsulates the repeated pattern across 19 teacher pages:
 ...
 *
 * @returns Teacher page setup props ready for TeacherLayout
 *
 * @example
 * ```tsx
 ...
 *   return (
 *     <TeacherLayout user={user ?? undefined} gamificationData={displayGamificationData}
 *       organizationName={user?.organization?.name || 'Mi Institucion'} onLogout={handleLogout}>
 *       {content}
 *     </TeacherLayout>
 *   );
 * }
 * ```
 */
export function useTeacherPageSetup() {
