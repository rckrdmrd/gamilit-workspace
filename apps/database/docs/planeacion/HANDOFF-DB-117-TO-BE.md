...
### 🆕 Validadores Adicionales (Corrección de Discrepancias)

Los siguientes validadores fueron agregados para resolver discrepancias entre los formatos implementados en frontend y los especificados originalmente:

#### 16. Detective Textual - Conexiones (`detective_textual` con `validate_detective_connections`)
...
```jsonb
{
  "connections": [
    ...
    {"from": "ev3", "to": "ev4", "relationship": "consecuencia"}
  ]
}
```

**Validación:**
- ✅ Verifica que cada conexión `(from, to, relationship)` coincida exactamente
- ✅ Todas las conexiones deben estar presentes
- ✅ Permite crédito parcial basado en número de conexiones correctas
...
#### 17. Predicción Narrativa - Escenarios (`prediccion_narrativa` con `validate_prediction_scenarios`)
...
**Validación:**
- ✅ Verifica que la predicción seleccionada para cada escenario sea correcta
- ✅ Formato similar a múltiple choice pero con escenarios
- ✅ Permite crédito parcial basado en número de escenarios correctos
...
#### 18. Causa-Efecto Matching (`construccion_hipotesis` con `validate_cause_effect_matching`)
...
- ✅ Cada causa puede tener múltiples consecuencias
- ✅ Permite crédito parcial basado en número de asociaciones correctas
...
**Nota importante:** Para usar estos validadores alternativos, se debe configurar en `exercise_validation_config` el campo `validation_function` con los nombres:
