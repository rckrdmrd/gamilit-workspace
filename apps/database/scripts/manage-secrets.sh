...
EOF
    ...
    # ========================================================================
    # NUEVO: Sincronizar archivos de database
    # ========================================================================

    # Extraer valores de .env del backend
    local DB_PASSWORD=$(grep "^DB_PASSWORD=" "$BACKEND_DIR/.env.$ENVIRONMENT" | cut -d= -f2)
    local DB_HOST=$(grep "^DB_HOST=" "$BACKEND_DIR/.env.$ENVIRONMENT" | cut -d= -f2)
    local DB_PORT=$(grep "^DB_PORT=" "$BACKEND_DIR/.env.$ENVIRONMENT" | cut -d= -f2)
    local DB_NAME=$(grep "^DB_NAME=" "$BACKEND_DIR/.env.$ENVIRONMENT" | cut -d= -f2)
    local DB_USER=$(grep "^DB_USER=" "$BACKEND_DIR/.env.$ENVIRONMENT" | cut -d= -f2)

    # Actualizar .env.database
    print_step "Actualizando database/.env.database..."
    local DB_ENV_FILE="$DATABASE_ROOT/.env.database"

    if [ -f "$DB_ENV_FILE" ]; then
        cp "$DB_ENV_FILE" "$DB_ENV_FILE.backup.$(date +%Y%m%d_%H%M%S)"
    fi

    cat > "$DB_ENV_FILE" << EOF
# ============================================================================
# GAMILIT Platform - Database Environment ($ENVIRONMENT)
# Generated by manage-secrets.sh: $(date)
# ⚠️  NO COMMITEAR - Contiene credenciales sensibles
# ============================================================================
...
EOF
    ...
    # Actualizar database-credentials-*.txt
    print_step "Actualizando database-credentials-${ENVIRONMENT}.txt..."
    local CREDS_FILE="$DATABASE_ROOT/database-credentials-${ENVIRONMENT}.txt"

    if [ -f "$CREDS_FILE" ]; then
        # Backup a credentials-backups
        mkdir -p "$DATABASE_ROOT/credentials-backups"
        cp "$CREDS_FILE" "$DATABASE_ROOT/credentials-backups/database-credentials-${ENVIRONMENT}.$(date +%Y%m%d_%H%M%S).txt"
    fi
...
EOF
    ...
    print_success "database-credentials-${ENVIRONMENT}.txt actualizado"
    ...
    print_info "  • $DATABASE_ROOT/.env.database"
    print_info "  • $DATABASE_ROOT/database-credentials-${ENVIRONMENT}.txt"
}
