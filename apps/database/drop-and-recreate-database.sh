#!/bin/bash
# ============================================================================
# GAMILIT - drop-and-recreate-database.sh (compat wrapper)
# ============================================================================
...
CANONICAL_SCRIPT="$SCRIPT_DIR/scripts/recreate-database.sh"
...
Uso (canónico recomendado):
  bash apps/database/scripts/recreate-database.sh --env dev --force

Compat wrapper (este script):
  ./drop-and-recreate-database.sh --env dev --force
  ./drop-and-recreate-database.sh --env prod --password "<pass>" --force
  ./drop-and-recreate-database.sh "<DATABASE_URL>"   # legacy
...
echo "[INFO] drop-and-recreate-database.sh es wrapper legacy -> scripts/recreate-database.sh"
