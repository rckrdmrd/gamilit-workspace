...
DROP TABLE IF EXISTS data_warehouse.dim_time CASCADE;
...
CREATE TABLE data_warehouse.dim_time (
    ...
    CONSTRAINT dim_time_full_time_key UNIQUE (full_time),
    CONSTRAINT dim_time_hour_24_check CHECK (hour_24 BETWEEN 0 AND 23),
    CONSTRAINT dim_time_hour_12_check CHECK (hour_12 BETWEEN 1 AND 12),
    CONSTRAINT dim_time_minute_check CHECK (minute BETWEEN 0 AND 59),
    CONSTRAINT dim_time_am_pm_check CHECK (am_pm IN ('AM', 'PM')),
    CONSTRAINT dim_time_period_check CHECK (period IN ('morning', 'afternoon', 'evening', 'night'))
);

-- =====================================================
-- INDEXES
-- =====================================================
CREATE INDEX idx_dim_time_hour ON data_warehouse.dim_time USING btree (hour_24);
CREATE INDEX idx_dim_time_period ON data_warehouse.dim_time USING btree (period);
CREATE INDEX idx_dim_time_school_hours ON data_warehouse.dim_time USING btree (is_school_hours) WHERE is_school_hours = TRUE;

-- =====================================================
-- OWNERSHIP AND PERMISSIONS
-- =====================================================
ALTER TABLE data_warehouse.dim_time OWNER TO gamilit_user;
...
COMMENT ON COLUMN data_warehouse.dim_time.time_key IS 'Surrogate key in HHMM integer format (e.g., 1430 = 2:30 PM)';
COMMENT ON COLUMN data_warehouse.dim_time.period IS 'Time of day: morning, afternoon, evening, night';
COMMENT ON COLUMN data_warehouse.dim_time.is_school_hours IS 'TRUE for 7:00-15:00 school hours';
