-- ============================================================================
-- TABLA: exercise_validation_configs
...
DROP TABLE IF EXISTS educational_content.exercise_validation_configs CASCADE;

CREATE TABLE educational_content.exercise_validation_configs (
...
-- COMENTARIOS
COMMENT ON TABLE educational_content.exercise_validation_configs IS
'Configuración de validación de respuestas por tipo de ejercicio. Define qué función SQL usar y sus parámetros.';

COMMENT ON COLUMN educational_content.exercise_validation_configs.exercise_type IS
'Tipo de ejercicio (crucigrama, verdadero_falso, etc.). Debe existir en ENUM educational_content.exercise_type';

COMMENT ON COLUMN educational_content.exercise_validation_configs.validation_function IS
'Nombre de la función SQL a llamar para validar (ej: validate_crucigrama, validate_true_false)';

COMMENT ON COLUMN educational_content.exercise_validation_configs.case_sensitive IS
'Si TRUE, la validación distingue entre mayúsculas y minúsculas';

COMMENT ON COLUMN educational_content.exercise_validation_configs.allow_partial_credit IS
'Si TRUE, permite puntuación parcial (ej: 7/10 respuestas = 70 puntos). Si FALSE, todo o nada.';

COMMENT ON COLUMN educational_content.exercise_validation_configs.fuzzy_matching_threshold IS
'Umbral de similaridad para fuzzy matching (0.00 = no match, 1.00 = match exacto). NULL = no usar fuzzy matching';

COMMENT ON COLUMN educational_content.exercise_validation_configs.normalize_text IS
'Si TRUE, normaliza texto antes de comparar (quita acentos, espacios extras, etc.)';

COMMENT ON COLUMN educational_content.exercise_validation_configs.special_rules IS
'Reglas específicas del tipo en formato JSONB (ej: {"allow_synonyms": true, "min_word_count": 50})';

COMMENT ON COLUMN educational_content.exercise_validation_configs.examples IS
...
CREATE INDEX idx_validation_config_type
    ON educational_content.exercise_validation_configs(exercise_type);

CREATE INDEX idx_validation_config_function
    ON educational_content.exercise_validation_configs(validation_function);

-- TRIGGER UPDATED_AT
CREATE TRIGGER trg_exercise_validation_configs_updated_at
    BEFORE UPDATE ON educational_content.exercise_validation_configs
    FOR EACH ROW
    EXECUTE FUNCTION gamilit.update_updated_at_column();

-- PERMISOS
GRANT SELECT ON educational_content.exercise_validation_configs TO authenticated;
GRANT ALL ON educational_content.exercise_validation_configs TO admin_teacher;
