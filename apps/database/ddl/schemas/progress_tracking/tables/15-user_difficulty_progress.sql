-- =====================================================
-- Table: user_difficulty_progresses
...
CREATE INDEX IF NOT EXISTS idx_user_difficulty_progresses_user
    ON progress_tracking.user_difficulty_progresses(user_id);

CREATE INDEX IF NOT EXISTS idx_user_difficulty_progresses_level
    ON progress_tracking.user_difficulty_progresses(difficulty_level);

CREATE INDEX IF NOT EXISTS idx_user_difficulty_progresses_success_rate
    ON progress_tracking.user_difficulty_progresses(difficulty_level, success_rate DESC);

CREATE INDEX IF NOT EXISTS idx_user_difficulty_progresses_ready_promotion
    ON progress_tracking.user_difficulty_progresses(user_id, is_ready_for_promotion)
    WHERE is_ready_for_promotion = TRUE;

-- Política RLS
ALTER TABLE progress_tracking.user_difficulty_progresses ENABLE ROW LEVEL SECURITY;

CREATE POLICY user_difficulty_progresses_select_own
ON progress_tracking.user_difficulty_progresses
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY user_difficulty_progresses_select_teacher
ON progress_tracking.user_difficulty_progresses
FOR SELECT
USING (
    EXISTS (
...
CREATE TRIGGER trg_user_difficulty_progresses_updated_at
    BEFORE UPDATE ON progress_tracking.user_difficulty_progresses
    FOR EACH ROW
    EXECUTE FUNCTION gamilit.update_updated_at_column();

-- Comentarios
COMMENT ON TABLE progress_tracking.user_difficulty_progresses IS
...
COMMENT ON COLUMN progress_tracking.user_difficulty_progresses.success_rate IS
'Tasa de éxito calculada automáticamente: (correct_first_attempt / attempted) * 100';

COMMENT ON COLUMN progress_tracking.user_difficulty_progresses.is_ready_for_promotion IS
'Flag que indica si el usuario cumple los criterios para promoción al siguiente nivel';

-- Grants
GRANT SELECT ON progress_tracking.user_difficulty_progresses TO authenticated;
