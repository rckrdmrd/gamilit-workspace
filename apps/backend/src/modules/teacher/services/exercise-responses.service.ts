...
export class ExerciseResponsesService {
   ...
   *
   * @example
   * const result = await service.getAttempts('teacher-uuid', 'tenant-uuid', {
   *   page: 1,
   *   limit: 20,
  ...
  async getAttemptsByStudent(
    ...
    await this.verifyTeacherAccess(teacherId, studentId, tenantId);
    ...
    const result = await this.getAttempts(teacherId, tenantId, query);
    return result.data;
  }
  ...
  async getExerciseResponses(
    ...
    return this.getAttempts(teacherId, tenantId, query);
  }
