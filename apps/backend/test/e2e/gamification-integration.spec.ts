...
describe('Gamification Integration (e2e)', () => {
  describe('Exercise -> Gamification Flow', () => {
    ...
    it('should track incremental achievement progress after exercise_completion activity', async () => {
      // Arrange: user has an in-progress achievement that matches exercise_completion
      const inProgressAchievement = createMockUserAchievement({
        id: 'ua-progress-1',
        progress: 3,
        max_progress: 5,
        is_completed: false,
      ...
      userAchievementRepo.find.mockResolvedValue([inProgressAchievement]);
      userAchievementRepo.save.mockImplementation((ua: any) => Promise.resolve(ua));

      // Act
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'verdadero_falso', moduleId: 'mod-1', score: 85 },
      );

      // Assert
      expect(updated).toHaveLength(1);
      expect(updated[0].progress).toBe(4);
      expect(updated[0].completion_percentage).toBe(80);
      expect(updated[0].is_completed).toBe(false);
    });

    it('should auto-complete achievement when incremental progress reaches max', async () => {
      // Arrange: user at 4/5, one more exercise completes it
      const nearComplete = createMockUserAchievement({
        id: 'ua-near-complete',
        progress: 4,
        max_progress: 5,
        is_completed: false,
      ...
      userAchievementRepo.find.mockResolvedValue([nearComplete]);
      userAchievementRepo.save.mockImplementation((ua: any) => Promise.resolve(ua));

      // For notification: fetch achievement by id
      achievementRepo.findOne.mockResolvedValue(nearComplete.achievement);

      // Act
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'sopa_letras', moduleId: 'mod-2', score: 100 },
      );
      ...
      expect(updated[0].completed_at).toBeInstanceOf(Date);
      expect(updated[0].completion_percentage).toBe(100);

      // WebSocket and notification should fire for the completion
      expect(wsService.emitAchievementUnlocked).toHaveBeenCalledWith(
        userId,
        expect.objectContaining({
          achievementId: 'ach-almost-done',
          title: 'Five Exercises',
        }),
      );
    });
    ...
    it('should handle perfect_score activity type for incremental progress', async () => {
      // Arrange: achievement tracks perfect scores
      const perfectScoreAchievement = createMockUserAchievement({
        progress: 2,
        max_progress: 5,
        is_completed: false,
        achievement: createMockAchievement({
          conditions: { type: 'perfect_score', requirements: { perfect_exercises: 5, score_required: 100 } },
        }),
      });

      userAchievementRepo.find.mockResolvedValue([perfectScoreAchievement]);
      userAchievementRepo.save.mockImplementation((ua: any) => Promise.resolve(ua));
    ...
    it('should handle streak activity type with absolute progress (not incremental)', async () => {
      // Arrange: streak achievement at 3/7
      const streakAchievement = createMockUserAchievement({
        progress: 3,
        max_progress: 7,
        is_completed: false,
        achievement: createMockAchievement({
      ...
      userAchievementRepo.find.mockResolvedValue([streakAchievement]);
      userAchievementRepo.save.mockImplementation((ua: any) => Promise.resolve(ua));

      // Act: streak updated to 5 days
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'streak',
        { currentStreak: 5 },
      ...
      // Assert: progress set to absolute 5, not incremented
      expect(updated).toHaveLength(1);
      expect(updated[0].progress).toBe(5);
      expect(updated[0].is_completed).toBe(false);
    });

    it('should not regress streak progress when current streak is lower than recorded', async () => {
      // Arrange: streak achievement already at 5/7 but streak broke back to 2
      const streakAchievement = createMockUserAchievement({
        progress: 5,
        max_progress: 7,
        is_completed: false,
      ...
      // Act: streak now at 2 (lower than recorded 5)
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'streak',
        { currentStreak: 2 },
      ...
      // Assert: should NOT update since 2 < 5
      expect(updated).toHaveLength(0);
      expect(userAchievementRepo.save).not.toHaveBeenCalled();
    });

    it('should skip inactive achievements during incremental progress', async () => {
      // Arrange: achievement with is_active=false
      const inactiveAchievement = createMockUserAchievement({
        progress: 2,
        max_progress: 5,
        is_completed: false,
        achievement: createMockAchievement({
      ...
      userAchievementRepo.find.mockResolvedValue([inactiveAchievement]);
      ...
      // Assert: inactive achievement not incremented
      expect(updated).toHaveLength(0);
      expect(userAchievementRepo.save).not.toHaveBeenCalled();
    });
  ...
  describe('Achievement Lifecycle', () => {
    ...
    it('should increment progress by 1 on each exercise_completion activity', async () => {
      // Arrange
      const ua = createMockUserAchievement({
        progress: 0,
        max_progress: 10,
        achievement: createMockAchievement({
          conditions: { type: 'exercise_completion', requirements: { exercises_completed: 10 } },
        }),
      });

      userAchievementRepo.find.mockResolvedValue([ua]);
      userAchievementRepo.save.mockImplementation((entity: any) => Promise.resolve(entity));

      // Act: 3 sequential increments
      await achievementsService.updateIncrementalProgress(userId, 'exercise_completion', {});

      // Assert
      expect(userAchievementRepo.save).toHaveBeenCalledTimes(1);
      const savedArg = userAchievementRepo.save.mock.calls[0][0];
      expect(savedArg.progress).toBe(1);
      expect(savedArg.completion_percentage).toBe(10);
    });
  ...
  describe('Cross-System: Exercise triggers multiple gamification subsystems', () => {
    ...
    it('should trigger both WebSocket and notification for achievement completion via incremental progress', async () => {
      // Arrange: achievement at 4/5, about to complete
      const achievementDef = createMockAchievement({
        id: 'ach-cross',
        name: 'Cross System Achievement',
        description: 'Tests cross-system flow',
        icon: 'star',
        conditions: { type: 'exercise_completion', requirements: { exercises_completed: 5 } },
      });
      ...
      userAchievementRepo.find.mockResolvedValue([ua]);
      userAchievementRepo.save.mockImplementation((entity: any) => Promise.resolve(entity));
      achievementRepo.findOne.mockResolvedValue(achievementDef);

      // Act
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'emparejamiento', moduleId: 'mod-1', score: 92 },
      );
      ...
      expect(wsService.emitAchievementUnlocked).toHaveBeenCalledWith(
        userId,
        expect.objectContaining({
          achievementId: 'ach-cross',
          title: 'Cross System Achievement',
          description: 'Tests cross-system flow',
          icon: 'star',
        }),
      );

      // Assert: notification created (with 2 args: payload + options)
      expect(notifService.create).toHaveBeenCalledWith(
        expect.objectContaining({
          userId,
          type: 'achievement_unlocked',
    ...
    it('should handle multiple achievements progressing from single exercise_completion', async () => {
      // Arrange: two different achievements match exercise_completion
      const ach1 = createMockAchievement({
        id: 'ach-multi-1',
        name: 'Exercise Starter',
        conditions: { type: 'exercise_completion', requirements: { exercises_completed: 3 } },
      });
      const ach2 = createMockAchievement({
        id: 'ach-multi-2',
        name: 'Progress Tracker',
        conditions: { type: 'progress', requirements: { exercises_completed: 10 } },
      });

      const ua1 = createMockUserAchievement({
        id: 'ua-multi-1',
        progress: 1,
        max_progress: 3,
        is_completed: false,
      ...
      const ua2 = createMockUserAchievement({
        id: 'ua-multi-2',
        progress: 5,
        max_progress: 10,
        is_completed: false,
      ...
      userAchievementRepo.find.mockResolvedValue([ua1, ua2]);
      userAchievementRepo.save.mockImplementation((entity: any) => Promise.resolve(entity));
      ...
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'timeline', moduleId: 'mod-3', score: 78 },
      );

      // Assert: both achievements progressed
      expect(updated).toHaveLength(2);
      expect(updated[0].progress).toBe(2);
      expect(updated[1].progress).toBe(6);
    });

    it('should handle module_first_exercise achievement type matching by moduleId', async () => {
      // Arrange: achievement for completing first exercise in module 'mod-M3'
      const moduleAch = createMockAchievement({
        id: 'ach-module-first',
        conditions: {
          type: 'module_first_exercise',
          requirements: { module_id: 'mod-M3', exercises_completed: 1 },
        },
      });
      ...
      const ua = createMockUserAchievement({
        ...
        max_progress: 1,
        is_completed: false,
        achievement: moduleAch,
      });

      userAchievementRepo.find.mockResolvedValue([ua]);
      userAchievementRepo.save.mockImplementation((entity: any) => Promise.resolve(entity));
      achievementRepo.findOne.mockResolvedValue(moduleAch);

      // Act: exercise from module mod-M3
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'detective_textual', moduleId: 'mod-M3', score: 95 },
      );

      // Assert: achievement completed (1/1)
      expect(updated).toHaveLength(1);
      expect(updated[0].progress).toBe(1);
      expect(updated[0].is_completed).toBe(true);
    });

    it('should NOT progress module_first_exercise for wrong module', async () => {
      // Arrange: achievement for module 'mod-M3' but exercise is from 'mod-M1'
      const moduleAch = createMockAchievement({
        conditions: {
          type: 'module_first_exercise',
          requirements: { module_id: 'mod-M3', exercises_completed: 1 },
      ...
      // Act: exercise from different module
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'sopa_letras', moduleId: 'mod-M1', score: 100 },
      );

      // Assert: no progress for wrong module
      expect(updated).toHaveLength(0);
      expect(userAchievementRepo.save).not.toHaveBeenCalled();
    });

    it('should handle exercise_score achievement type matching by exerciseType', async () => {
      // Arrange: achievement for scoring on 'crucigrama' exercises
      const scoreAch = createMockAchievement({
        conditions: {
          type: 'exercise_score',
          requirements: { exercise_type: 'crucigrama', min_score: 90 },
        },
      });
      ...
      const ua = createMockUserAchievement({
        ...
        max_progress: 5,
        is_completed: false,
        achievement: scoreAch,
      });

      userAchievementRepo.find.mockResolvedValue([ua]);
      userAchievementRepo.save.mockImplementation((entity: any) => Promise.resolve(entity));

      // Act: crucigrama exercise completed
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'crucigrama', moduleId: 'mod-1', score: 95 },
      );
    ...
    it('should NOT progress exercise_score for non-matching exercise type', async () => {
      // Arrange: achievement for 'crucigrama' but exercise is 'sopa_letras'
      const scoreAch = createMockAchievement({
        conditions: {
          type: 'exercise_score',
          requirements: { exercise_type: 'crucigrama', min_score: 90 },
      ...
      // Act: different exercise type
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        { exerciseType: 'sopa_letras', moduleId: 'mod-1', score: 100 },
      );
    ...
    it('should handle module_completion activity type', async () => {
      // Arrange
      const moduleAch = createMockAchievement({
        conditions: { type: 'module_completion', requirements: { module_id: 'mod-1', completion_percentage: 100 } },
      });
    ...
    it('should gracefully handle errors in incremental progress without throwing', async () => {
      // Arrange: simulate a repo error
      userAchievementRepo.find.mockRejectedValue(new Error('DB connection lost'));

      // Act: should NOT throw, should return empty array
      const updated = await achievementsService.updateIncrementalProgress(
        userId,
        'exercise_completion',
        {},
