...
### Estructura de Documentación - Carpetas Principales

| Carpeta Real | Propósito | Cuándo Consultar | Cuándo Actualizar |
|--------------|-----------|------------------|-------------------|
| **01-fase-alcance-inicial/** | Requerimientos base (EAI-001 a EAI-008) | **ANTES** de análisis y planificación | **Nunca** (solo Product Owner) |
| **02-fase-robustecimiento/** | Requerimientos de mejora (EMR-*) | **ANTES** de análisis | **Nunca** (solo Product Owner) |
| **03-fase-extensiones/** | Extensiones (EXT-001 a EXT-011) | **ANTES** de análisis | **Nunca** (solo Product Owner) |
| **90-transversal/** | Specs técnicas (API, DB, arquitectura, SSOT) | **DURANTE** análisis y planificación | **SIEMPRE** después de implementar |
| **planning/** | Planificación y avance del proyecto | **ANTES** de planificar, **DURANTE** ejecución | **SIEMPRE** al actualizar progreso |
| **95-guias-desarrollo/** | Guías y documentación de desarrollo | **DESPUÉS** de implementación (para validar) | **SIEMPRE** después de implementar |

### Carpetas Adicionales

| Carpeta | Propósito | Uso por Agentes |
|---------|-----------|-----------------|
| **00-vision-general/** | Visión general, onboarding, glosario | Consultar al inicializar agente por primera vez |
| **96-quick-reference/** | Cheatsheets y guías rápidas | Consultar para referencias rápidas |
| **97-adr/** | Architecture Decision Records | Consultar antes de decisiones arquitectónicas |
| **90-transversal/ssot/** | Single Source of Truth (SSOT) | Consultar para definiciones canónicas |

### Mapeo de Rutas (Referencia Rápida)

```yaml
# Mapeo: Convención Anterior → Ruta Real
requerimientos: "01-fase-alcance-inicial/, 02-fase-robustecimiento/, 03-fase-extensiones/"
especificaciones_tecnicas: "90-transversal/"
apis: "90-transversal/api/"
database_specs: "90-transversal/inventarios-database/"
arquitectura: "90-transversal/arquitectura/"
ssot: "90-transversal/ssot/"
planificacion: "planning/"
guias_desarrollo: "95-guias-desarrollo/"
adr: "97-adr/"
```
...
cat ../docs/01-fase-alcance-inicial/{archivo-especifico}.md
```
...
4. Lee: docs/01-fase-alcance-inicial/gamificacion/_MAP.md
5. Encuentra: BADGES-SISTEMA.md
6. Lee SOLO: docs/01-fase-alcance-inicial/gamificacion/BADGES-SISTEMA.md
...
**Validar contra:**
- `/docs/01-fase-alcance-inicial/` (¿Está definido el requerimiento?)
- `/docs/90-transversal/` (¿Cómo debe implementarse?)
- `/docs/planning/` (¿Está en el plan actual?)

**Preguntas obligatorias:**
- [ ] ¿Este requerimiento está documentado en `01-fase-alcance-inicial/`?
- [ ] ¿Las especificaciones técnicas existen en `90-transversal/`?
- [ ] ¿Esta tarea está en la planificación actual de `planning/`?
...
**Fase:** Ejecución
**Validar contra:**
- `/docs/90-transversal/` (¿Estoy siguiendo las specs?)
- `/docs/95-guias-desarrollo/` (¿Estoy siguiendo los estándares?)
...
**Fase:** Completitud
**Validar contra:**
- `/docs/01-fase-alcance-inicial/` (¿Cumplí el requerimiento?)
- `/docs/90-transversal/` (¿Implementé según specs?)

**Actualizar obligatoriamente:**
- ✅ `/docs/95-guias-desarrollo/` - Documentar lo implementado
- ✅ `/docs/planning/` - Actualizar progreso/estado

**Preguntas obligatorias:**
- [ ] ¿La implementación cumple con el requerimiento original?
- [ ] ¿Todos los criterios de aceptación están cubiertos?
- [ ] ¿He documentado lo implementado en `95-guias-desarrollo/`?
- [ ] ¿He actualizado el progreso en `planning/`?
...
| Acción | Actualizar en `/docs/planning/` |
|--------|------------------------------------------|
...
**Situación:** Usuario pide implementar algo que no está en `/docs/01-fase-alcance-inicial/`

**Acción:**
1. 🛑 **DETENER** implementación
2. Documentar en `orchestration/01-analisis/missing-requirements/YYYY-MM-DD-{feature}.md`
3. Preguntar al usuario:
   > "No encuentro el requerimiento para '{feature}' en `/docs/01-fase-alcance-inicial/`.
...
**Situación:** `/docs/01-fase-alcance-inicial/` dice A, pero `/docs/90-transversal/` dice B
...
3. Notificar al usuario:
   ...
   > - `/docs/01-fase-alcance-inicial/{archivo}.md` (dice A)
   > - `/docs/90-transversal/{archivo}.md` (dice B)
   > ¿Cuál debo seguir?"
...
**Situación:** El código actual no coincide con `/docs/95-guias-desarrollo/`
...
5. Proponer al usuario:
   > "La documentación en `/docs/95-guias-desarrollo/{archivo}.md` está desactualizada.
...
**Situación:** Las specs en `/docs/90-transversal/` son ambiguas o incompletas

**Acción:**
1. Documentar ambigüedad en `orchestration/01-analisis/ambiguities/YYYY-MM-DD-{tema}.md`
2. Listar interpretaciones posibles
3. Preguntar al usuario:
   > "Las especificaciones para '{feature}' en `/docs/90-transversal/{archivo}.md` son ambiguas.
...
- [ ] Busqué el requerimiento en `docs/01-fase-alcance-inicial/` usando `_MAP.md`
- [ ] Encontré el requerimiento documentado
- [ ] Leí las especificaciones técnicas en `docs/90-transversal/`
- [ ] Verifiqué ADRs relevantes en `docs/97-adr/`
- [ ] Verifiqué estándares aplicables en `docs/95-guias-desarrollo/`
...
- [ ] El plan está alineado con `docs/01-fase-alcance-inicial/`
- [ ] El plan sigue las specs de `docs/90-transversal/`
- [ ] El plan está documentado en `orchestration/02-planes/`
- [ ] Verifiqué que está en `docs/planning/` como tarea actual
- [ ] Si no está en planificación → Notifiqué al usuario

### Checklist: Ejecución

- [ ] Cada microciclo valido contra `docs/90-transversal/`
...
- [ ] Validé que cumple con `docs/01-fase-alcance-inicial/`
- [ ] Validé que sigue `docs/90-transversal/`
- [ ] Documenté lo implementado en `docs/95-guias-desarrollo/`
- [ ] Actualicé progreso en `docs/planning/`
...
grep -r "nombre-modulo" ../docs/

# Buscar referencias en documentación de desarrollo
grep -r "nombre-feature" ../docs/95-guias-desarrollo/
```
...
find ../docs/ -type d -exec bash -c 'count=$(ls -1 "$0"/*.md 2>/dev/null | wc -l); if [ $count -gt 3 ] && [ ! -f "$0/_MAP.md" ]; then echo "Falta _MAP.md en: $0"; fi' {} \;
```
...
| **% Documentación en `95-guias-desarrollo/` actualizada** | 100% | Verificar que cada implementación tenga entrada |
| **% Planificación en `planning/` actualizada** | 100% | Verificar estado de tareas |
...
4. ✅ Se documentó lo implementado en `docs/95-guias-desarrollo/`
5. ✅ Se actualizó progreso en `docs/planning/`
