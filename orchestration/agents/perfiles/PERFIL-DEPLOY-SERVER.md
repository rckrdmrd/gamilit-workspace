...
**Causa raíz documentada:** PostgreSQL estuvo no disponible ~8 minutos durante aplicación de DDL. El budget de reintentos de TypeORM (5 intentos x 5 s = 25 s) se agotó muy antes de la recuperación. PM2 con `max_restarts: 10` acumuló ~10-16 reinicios en esa ventana, marcó el proceso como "errored" y detuvo el auto-restart. Tras la recuperación de PostgreSQL, el backend NO se reinició solo.
...
# Verificar UUID chain (usuarios con perfil correspondiente)
sudo -u postgres psql -d gamilit_platform -c \
