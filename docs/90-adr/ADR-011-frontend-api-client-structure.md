...
- `lib/api/auth.api.ts` -> `features/auth/api/authAPI.ts`

All import paths using `@/lib/api/` should now use `@/services/api/` or `@/features/*/api/` accordingly. The architectural decisions in this ADR remain valid; only the file locations have changed.
