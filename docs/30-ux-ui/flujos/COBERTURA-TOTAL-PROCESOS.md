...
| FL-STU-05 | Student | Perfil y ajustes del estudiante | EPIC-GAM-F3-PROFILES | `apps/student/pages/SettingsPage.tsx` | editar perfil/preferencias | `/api/v1/profile/:userId`, `/api/auth/profile` | `auth_management.profiles`, `auth.users` | `shared/FLUJO-PERFIL-CONFIGURACION.md` |
| FL-STU-06 | Student | Dashboard y progreso academico | EPIC-GAM-F1-ANALYTICS | `apps/student/pages/DashboardComplete.tsx` | filtros/consultas dashboard | `/api/v1/progress/*`, `/api/v1/gamification/*` | `progress_tracking.*`, `analytics.*`, `gamification_system.user_stats` | `student/FLUJO-EJERCICIO-COMPLETO.md` |
...
| FL-TCH-04 | Teacher | Configuracion docente y mensajeria | EPIC-GAM-F3-TEACHER-PORTAL | `apps/teacher/pages/TeacherSettingsPage.tsx` | actualizar preferencias/canales | `/api/v1/teacher/settings/*`, `/api/v1/messages/*` | `auth_management.profiles`, `communication.*` | `shared/FLUJO-PERFIL-CONFIGURACION.md` |
...
| FL-SHR-02 | Shared | Sesion/seguridad y recuperacion de acceso | EPIC-GAM-F1-AUTH | login/reset flows | acciones de seguridad de cuenta | `/auth/*` | `auth.users`, `auth_management.*` | `auth/FLUJO-REGISTRO-LOGIN.md`, `auth/FLUJO-RECUPERACION-PASSWORD.md` |
