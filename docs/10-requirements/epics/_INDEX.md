# Epics — gamilit

> Indice de epicas del proyecto gamilit (Plataforma Educativa Gamificada)

**SSOT:** Este directorio es la fuente unica de documentacion narrativa de epicas del proyecto.
**Tracking:** [orchestration/work-items/epics/](../../../orchestration/work-items/epics/)
**ADR:** [ADR-0019](../../90-adr/ADR-0019-ssot-documentacion-producto-en-proyecto.md)

---

## Wave 3 — EPICs Tecnicas (COMPLETADAS)

> 11 EPICs por capa tecnica. Todas completadas. [Ver detalle](_wave-3-technical/_INDEX.md)

| Epic ID | Titulo | SP | Estado | EPIC.md |
|---------|--------|-----|--------|---------|
| EPIC-GAM-SCAFFOLD | Scaffolding Gamilit | 5 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-SCAFFOLD/EPIC.md) |
| EPIC-GAM-REQUIREMENTS | Requerimientos Gamilit | 13 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-REQUIREMENTS/EPIC.md) |
| EPIC-GAM-ARCHITECTURE | Arquitectura Gamificacion | 13 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-ARCHITECTURE/EPIC.md) |
| EPIC-GAM-DATABASE | Esquema BD Gamilit | 21 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-DATABASE/EPIC.md) |
| EPIC-GAM-BACKEND | Backend Gamilit | 34 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-BACKEND/EPIC.md) |
| EPIC-GAM-FRONTEND | Frontend Gamilit | 34 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-FRONTEND/EPIC.md) |
| EPIC-GAM-K8S | Kubernetes Gamilit | 8 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-K8S/EPIC.md) |
| EPIC-GAM-TESTING | Tests Gamilit | 13 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-TESTING/EPIC.md) |
| EPIC-GAM-DEVOPS | DevOps Gamilit | 8 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-DEVOPS/EPIC.md) |
| EPIC-GAM-DOCS | Documentacion Gamilit | 8 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-DOCS/EPIC.md) |
| EPIC-GAM-INTEGRATION | Integracion Gamilit | 5 | completed | [EPIC.md](_wave-3-technical/EPIC-GAM-INTEGRATION/EPIC.md) |

**Subtotal Wave 3:** 11 EPICs | 162 SP | 100% completado

---

## F1 — Alcance Inicial (Funcionales)

> 7 EPICs por feature funcional. Fase de alcance inicial del producto.

| Epic ID | Titulo | SP | Estado | EPIC.md |
|---------|--------|-----|--------|---------|
| EPIC-GAM-F1-AUTH | Autenticacion y Fundamentos | 34 | completed | [EPIC.md](EPIC-GAM-F1-AUTH/EPIC.md) |
| EPIC-GAM-F1-EXERCISES | Ejercicios y Actividades | 34 | completed | [EPIC.md](EPIC-GAM-F1-EXERCISES/EPIC.md) |
| EPIC-GAM-F1-GAMIFICATION | Sistema de Gamificacion | 34 | completed | [EPIC.md](EPIC-GAM-F1-GAMIFICATION/EPIC.md) |
| EPIC-GAM-F1-ANALYTICS | Analytics y Reportes | 21 | completed | [EPIC.md](EPIC-GAM-F1-ANALYTICS/EPIC.md) |
| EPIC-GAM-F1-ADMIN | Administracion Base | 21 | completed | [EPIC.md](EPIC-GAM-F1-ADMIN/EPIC.md) |
| EPIC-GAM-F1-CONFIG | Configuracion del Sistema | 8 | completed | [EPIC.md](EPIC-GAM-F1-CONFIG/EPIC.md) |
| EPIC-GAM-F1-PORTAL-ADMIN | Portal Administrador | 78 | completed | [EPIC.md](EPIC-GAM-F1-PORTAL-ADMIN/EPIC.md) |

**Subtotal F1:** 7 EPICs | 230 SP | 100% completado

---

## F2 — Robustecimiento (Funcionales)

> 3 EPICs de consolidacion tecnica y mejoras.

| Epic ID | Titulo | SP | Estado | EPIC.md |
|---------|--------|-----|--------|---------|
| EPIC-GAM-F2-MODULES-M4M5 | Modulos M4 y M5 | 21 | completed | [EPIC.md](EPIC-GAM-F2-MODULES-M4M5/EPIC.md) |
| EPIC-GAM-F2-DB-MIGRATION | Migracion BD | 13 | completed | [EPIC.md](EPIC-GAM-F2-DB-MIGRATION/EPIC.md) |
| EPIC-GAM-F2-TECH-CONSOLIDATION | Consolidacion Tecnica | 8 | completed | [EPIC.md](EPIC-GAM-F2-TECH-CONSOLIDATION/EPIC.md) |

**Subtotal F2:** 3 EPICs | 42 SP | 100% completado

---

## F3 — Extensiones (Funcionales)

> 12 EPICs de extensiones y nuevas funcionalidades.

| Epic ID | Titulo | SP | Estado | EPIC.md |
|---------|--------|-----|--------|---------|
| EPIC-GAM-F3-SOCIAL-GAMIFICATION | Gamificacion Social | 13 | in_progress | [EPIC.md](EPIC-GAM-F3-SOCIAL-GAMIFICATION/EPIC.md) |
| EPIC-GAM-F3-TEACHER-PORTAL | Portal Maestros | 34 | in_progress | [EPIC.md](EPIC-GAM-F3-TEACHER-PORTAL/EPIC.md) |
| EPIC-GAM-F3-ADMIN-EXTENDED | Admin Extendido | 34 | in_progress | [EPIC.md](EPIC-GAM-F3-ADMIN-EXTENDED/EPIC.md) |
| EPIC-GAM-F3-NOTIFICATIONS | Notificaciones | 8 | in_progress | [EPIC.md](EPIC-GAM-F3-NOTIFICATIONS/EPIC.md) |
| EPIC-GAM-F3-PROFILES | Perfiles | 8 | planned | [EPIC.md](EPIC-GAM-F3-PROFILES/EPIC.md) |
| EPIC-GAM-F3-REPORTS | Reportes | 8 | planned | [EPIC.md](EPIC-GAM-F3-REPORTS/EPIC.md) |
| EPIC-GAM-F3-CONTENT | Contenido | 8 | planned | [EPIC.md](EPIC-GAM-F3-CONTENT/EPIC.md) |
| EPIC-GAM-F3-LTI | Integracion LTI | 13 | planned | [EPIC.md](EPIC-GAM-F3-LTI/EPIC.md) |
| EPIC-GAM-F3-WHITE-LABEL | White Label | 13 | planned | [EPIC.md](EPIC-GAM-F3-WHITE-LABEL/EPIC.md) |
| EPIC-GAM-F3-PEER-CHALLENGES | Retos Entre Pares | 8 | planned | [EPIC.md](EPIC-GAM-F3-PEER-CHALLENGES/EPIC.md) |
| EPIC-GAM-F3-PARENT-NOTIFICATIONS | Notificaciones Padres | 5 | planned | [EPIC.md](EPIC-GAM-F3-PARENT-NOTIFICATIONS/EPIC.md) |
| EPIC-GAM-F3-PARENT-PORTAL | Portal Padres | 13 | planned | [EPIC.md](EPIC-GAM-F3-PARENT-PORTAL/EPIC.md) |

**Subtotal F3:** 12 EPICs | 165 SP | ~33% en progreso

---

## F4 — Validacion (QA/Integracion)

> 1 EPIC de validacion e integracion integral del sistema.

| Epic ID | Titulo | SP | Estado | EPIC.md |
|---------|--------|-----|--------|---------|
| EPIC-GAM-F4-VALIDATION | Validacion e Integracion Integral | 89 | in_progress | [EPIC.md](EPIC-GAM-F4-VALIDATION/EPIC.md) |

**Subtotal F4:** 1 EPIC | 89 SP | En progreso

---

## Resumen General

| Wave/Fase | EPICs | SP | Estado |
|-----------|-------|-----|--------|
| Wave 3 (Tecnicas) | 11 | 162 | Completado |
| F1 (Alcance Inicial) | 7 | 230 | Completado |
| F2 (Robustecimiento) | 3 | 42 | Completado |
| F3 (Extensiones) | 12 | 165 | En progreso |
| F4 (Validacion) | 1 | 89 | En progreso |
| **Total** | **34** | **688** | |

---

*Actualizado: 2026-02-10 | EPIC-GAM-F4-VALIDATION*
