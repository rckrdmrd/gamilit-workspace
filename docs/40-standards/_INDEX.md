# 40 - Estandares

> Guias de estilo, convenciones de codigo y estandares de calidad.

---

## Tabla de Contenidos (17 Estandares Activos)

| # | Estandar | Proposito |
|---|----------|-----------|
| 1 | ESTANDAR-API.md | Convenciones RESTful, Swagger, seguridad de endpoints |
| 2 | ESTANDAR-BACKEND-PROFESIONAL.md | Redirect a backend-profesional/ (8 modulos SOLID/DDD) |
| 3 | ESTANDAR-CODIGO.md | Convenciones generales de codigo |
| 4 | ESTANDAR-DATABASE-PROFESIONAL.md | PostgreSQL 15, DDL, RLS, triggers |
| 5 | ESTANDAR-DIAGRAMAS-ER.md | Convenciones para diagramas ER |
| 6 | ESTANDAR-DOCUMENTACION.md | Documentacion del proyecto |
| 7 | ESTANDAR-FRONTEND-PROFESIONAL.md | React 19, TypeScript, componentes |
| 8 | ESTANDAR-GIT.md | Commits, branching, PRs |
| 9 | ESTANDAR-MEMORIA-TOKENS.md | Gestion de ventana de contexto IA |
| 10 | ESTANDAR-NOMENCLATURA.md | Nomenclatura general |
| 11 | ESTANDAR-NOMENCLATURA-API.md | snake_case/camelCase entre capas |
| 12 | ESTANDAR-PERFORMANCE.md | Optimizacion y rendimiento |
| 13 | ESTANDAR-SEGURIDAD.md | RLS, JWT, rate limiting |
| 14 | ESTANDAR-SKILLS.md | Skills/comandos de agentes |
| 15 | ESTANDAR-TESTING.md | Unit, integration, E2E (piramide 70-20-10) |
| 16 | ESTANDAR-OBSERVABILIDAD.md | OpenTelemetry, Prometheus, tracing, SLOs |
| 17 | ESTANDAR-12-FACTOR-APP.md | Checklist 12-Factor App compliance |

## Subdirectorio: backend-profesional/ (8 modulos)

| # | Modulo | Tema |
|---|--------|------|
| 1 | 01-principios-solid.md | Principios SOLID aplicados a NestJS |
| 2 | 02-clean-architecture.md | Clean Architecture patterns |
| 3 | 03-repository-pattern.md | Repository pattern con TypeORM |
| 4 | 04-domain-driven-design.md | DDD aplicado al proyecto |
| 5 | 05-manejo-errores.md | Error handling centralizado |
| 6 | 06-validacion-datos.md | Validacion con class-validator |
| 7 | 07-testing-patterns.md | Patrones de testing backend |
| 8 | 08-referencias.md | Referencias y recursos |

---

*Ultima actualizacion: 2026-02-14*
